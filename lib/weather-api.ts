...
export async function getMonthlyPrediction(lat: number, lon: number, monthOffset: number): Promise<MonthlyPrediction> {
  const coords = normalizeCoordinates(lat, lon)
  console.log(`[v0] Generating monthly prediction for ${monthOffset} month(s) ahead`)
  ...
  try {
    ...
    const historicalAvgTemp = historicalData.reduce((sum, day) => sum + day.temp, 0) / historicalData.length
    const historicalAvgWind = historicalData.reduce((sum, day) => sum + day.wind_speed, 0) / historicalData.length
    const historicalAvgHumidity = historicalData.reduce((sum, day) => sum + day.humidity, 0) / historicalData.length
    ...
    const predictedMinTemp = predictedAvgTemp - (5 + Math.random() * 2) * uncertaintyFactor
    const predictedMaxTemp = predictedAvgTemp + (5 + Math.random() * 2) * uncertaintyFactor
    const predictedAvgWind = Math.max(0, historicalAvgWind * seasonalWindFactor + (Math.random() - 0.5) * 1)
    const predictedAvgHumidity = Math.max(
      30,
      Math.min(85, historicalAvgHumidity + seasonalHumidityAdjustment + (Math.random() - 0.5) * 10),
    )
    ...
    const precipitationDays = Math.round(basePrecipitationDays + Math.random() * 8) // Variation
    const sunnyDays = Math.max(5, 30 - precipitationDays - Math.round(Math.random() * 5))
    ...
    if (tempAdjustment > 3) {
      pattern += "Significantly warmer than average temperatures expected with "
    } else if (tempAdjustment > 1) {
      pattern += "Warmer than average temperatures expected with "
    } else if (tempAdjustment < -3) {
      pattern += "Significantly cooler than average temperatures expected with "
    } else if (tempAdjustment < -1) {
    ...
    if (precipitationDays > 15) {
      ...
      pattern += "significantly below-average rainfall and mostly dry conditions."
    ...
    if (monthOffset > 6) {
      recommendations.push("Note: Long-range predictions have higher uncertainty - check closer to date")
    }

    if (predictedAvgTemp > 30) {
      recommendations.push("Expect very hot conditions - plan activities during cooler hours")
      recommendations.push("Ensure adequate hydration and sun protection measures")
    } else if (predictedAvgTemp > 25) {
      recommendations.push("Plan activities during cooler morning or evening hours")
      recommendations.push("Ensure adequate hydration and sun protection")
    } else if (predictedAvgTemp < 5) {
      recommendations.push("Expect very cold conditions - dress in multiple warm layers")
      recommendations.push("Consider indoor alternatives during coldest periods")
      ...
      recommendations.push("Excellent temperature range for most outdoor activities")
    }

    if (precipitationDays > 15) {
      recommendations.push("Expect frequent rain - have backup indoor plans ready")
      ...
      recommendations.push("Waterproof gear recommended for outdoor activities")
    } else if (precipitationDays < 6) {
      recommendations.push("Excellent dry conditions expected for outdoor planning")
    } else {
      recommendations.push("Generally favorable conditions for outdoor planning")
    }

    if (predictedAvgWind > 12) {
      recommendations.push("Expect strong winds - avoid exposed areas and secure equipment")
    } else if (predictedAvgWind > 8) {
      recommendations.push("Be prepared for windy conditions, especially in exposed areas")
    }
    ...
    console.error(`[v0] Error generating monthly prediction:`, error)
    throw error
  }
}
