    ...
    <div className="text-center mb-8">
      ...
      <div className="flex items-center justify-center gap-4">
        <p className="text-muted-foreground text-lg text-pretty">
          NASA Space Apps Challenge 2025 • Advanced Weather Risk Assessment
          System
        </p>
        <a
      ...
      {weatherAnomalies.length > 0 && (
        <div className="mt-4 p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
          <p className="text-destructive font-medium">
            ⚠️ Weather Anomalies Detected
          </p>
          <p className="text-sm text-destructive/80">
            {weatherAnomalies.length} unusual weather pattern
            {weatherAnomalies.length > 1 ? 's' : ''} identified
          </p>
        </div>
      )}
    </div>
